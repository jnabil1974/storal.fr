    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 via-white to-slate-100">
      ...
      <header className="bg-gradient-to-r from-orange-600 to-red-600 text-white py-12">
        <div className="max-w-6xl mx-auto px-4">
          <Link href="/" className="text-orange-100 hover:text-white mb-4 inline-block transition">
            ← Retour à l'accueil
          </Link>
            ...
            {subcategories.map((sub) => (
              <Link key={sub.id} href={`/products/store-antichaleur/${sub.slug}`}>
                <div className="group bg-white rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 cursor-pointer h-full flex flex-col overflow-hidden border border-slate-100">
                  <div className="w-full h-56 bg-gradient-to-br from-orange-50 to-red-50 flex items-center justify-center overflow-hidden relative">
                    {sub.imageUrl ? (
                      <Image
                        src={sub.imageUrl}
                      ...
                      <svg className="w-24 h-24 text-orange-400 group-hover:text-orange-600 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z" />
                      </svg>
                    )}
                  </div>
                  <div className="p-6 flex flex-col flex-grow">
                    <span className="inline-block w-fit bg-orange-600 text-white text-xs font-bold px-3 py-1 rounded-full mb-3">
                      Été ☀️
                    </span>
